onDOMReady = ->
  ...
  $(document).on 'click', '#selection-toggle', ->
  ...
  if $(".batch_actions_selector").length && $(":checkbox.toggle_all").length
    ...
    $(document).on 'change', '.paginated_collection :checkbox', ->
      checkboxLength = $(".paginated_collection :checkbox:checked").length
      if checkboxLength
        $('body').addClass 'batch-enabled'
        $('#batch_bar #page_title').find('.count').html checkboxLength
      else
        $('body').removeClass 'batch-enabled'
